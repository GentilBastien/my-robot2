...
describe('HexagonalGridStructure', () => {
  ...
  test('HexagonalGrid possiblePaths #4', () => {
    const grid = produceCustomGrid();
    const start: HexagonalCellStructure<Weight> = grid.getCellAt({ x: 2, y: 3, z: -5 });
    const a = grid.possiblePaths(start, 2);
    const result = a.map(path => path.coordinatesPath);
    console.log(a.map(path => path.costs));
    const expected = [
      [{ x: 2, y: 3, z: -5 }],
      [
        { x: 2, y: 3, z: -5 },
